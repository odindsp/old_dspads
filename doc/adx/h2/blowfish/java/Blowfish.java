...
public class Blowfish {
	...
	private  int m_chain_xl;
	private  int m_chain_xr;

	private  int m_crypt_xl;
	private  int m_crypt_xr;
	...
	public byte[] encrypt(final byte[] source, final int offset,
		...
		String strOut = new String(Base64.encode(output));  
		
		//安全替换
		String step1 = strOut.replace("+", "-");
		String step2 = step1.replace("/", "_");
		
		return step2.getBytes();
	}
	...
	public byte[] decryptBytes(final byte[] source) throws UnsupportedEncodingException {
		encrypting = false;
		byte[] src_base64 = null;
		
		String decode_base64 = new String(source,"UTF-8");
	//	String step1 = decode_base64.replace("-", "+");
	//	String step2 = step1.replace("_", "/");
		
		src_base64 = Base64.decode(decode_base64.toCharArray());
		/*
		//先进行base64解密
		try {
			src_base64 = Base64.decode(step2.toCharArray());
		} catch (UnsupportedEncodingException e) {
			return null;
		}
		*/

		final int length = src_base64.length;
		final int blocks = (length / BLOCK_SIZE)
				+ (length % BLOCK_SIZE != 0 ? 1 : 0);
		...
		for (int i = 0; i < blocks; i++) {
			processBlock(src_base64, inputOffset, output, outputOffset);
			inputOffset += BLOCK_SIZE;
			outputOffset += BLOCK_SIZE;
		}
	...
	private void decryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex) {
		...
		m_chain_xl = m_crypt_xl;
		m_chain_xr = m_crypt_xr;
	...
	public static void main(String args[]) throws Exception {
		...
		String en_price= URLDecoder.decode("yVfYL1nvd0M=", "UTF-8");
		
		String token = "b82de03b-3918-43e4-9469-0a66f27c57a3";		
	
		byte[] tokendata = token.getBytes("UTF-8");

		Blowfish bf2 = new Blowfish();
		bf2.init(true, tokendata);
		String decode = bf2.decryptString(en_price.getBytes("UTF-8"));
