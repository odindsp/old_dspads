...
static int parseAmaxRequest(RECVDATA *recvdata, string &requestid)
{
	json_t *root = NULL, *label;
	int err = E_SUCCESS;
#ifndef DEBUG
	writeLog(g_logid_local, LOGINFO, "Begin parseAmaxRequest");
#endif
...
#ifndef DEBUG
	writeLog(g_logid_local, LOGINFO, "End parseAmaxRequest");
#endif
exit:
...
static bool setAmaxJsonResponse(string requsetid, string &data_out)
{
	char *text;
...
#ifndef DEBUG
	writeLog(g_logid_local, LOGINFO, "Begin setAmaxJsonResponse");
#endif
...
int getBidResponse(IN RECVDATA *recvdata, OUT string &senddata)
{
	...
	ret = parseAmaxRequest(recvdata, requestid);
	if(ret == E_BAD_REQUEST)
		goto exit;

	setAmaxJsonResponse(requestid, output);
