...
static void *doit(void *arg)
{
	...
	FLOW_BID_INFO flow_bid(ADX_INMOBI);
	...
	while (run_flag)
	{
		...
		if (strcmp(method, "POST") != 0)
		{
			...
			cflog(g_logid_local, LOGERROR, "remoteaddr: %s, REQUEST_METHOD: %s is not POST.",
				remoteaddr, method);
			flow_bid.set_err(E_REQUEST_HTTP_METHOD);
			goto loopend;
		}
		...
		{
			cflog(g_logid_local, LOGERROR, "remoteaddr: %s, CONTENT_TYPE: %s, Wrong Content-Type",
				remoteaddr, contenttype);
			flow_bid.set_err(E_REQUEST_HTTP_CONTENT_TYPE);
			goto loopend;
		}
		...
		if (errcode == E_INVALID_PARAM) // 正常竞价不会发生
			goto loopend;
		...
		if (fullreqrecord)
		{
			cflog(g_logid_local_traffic, LOGMAX, "requestid: %s, in: %s, out: %s",
				requestid.c_str(), recvdata->data, senddata.c_str());
		}
		...
		if (errcode == E_SUCCESS)
		{
			...
			cflog(g_logid_response, LOGINFO, "%s", response_short.c_str());
...
{ /* 信号处理例程，其中dunno将会得到信号的值 */
	...
	case SIGTERM://SIGTERM
		...
		/*	if (geodb != 0)
			...
			cout<<"sigroutine bid_filter_uninitialize, errcode:0x"<< dec << errcode << endl;
			g_ctx = 0;
...
int main(int argc, char *argv[])
{
	...
	map<string, vector<uint32_t> >::const_iterator it;
	...
	for (it = app_cat_table_adx2com.begin(); it != app_cat_table_adx2com.end(); ++it)
	{
		...
		{
			val += string(" ") + hexToString(it->second[i]);
		}
	...
	MD5_Init(&hash_ctx);
	errcode = bid_filter_initialize(g_logid_local, ADX_INMOBI, cpu_count, &run_flag,
		private_conf, thread_id, &g_ctx);
	if (errcode != E_SUCCESS)
	{
		FAIL_SHOW;
	...
	errcode = g_ser_log.init(cpu_count, ADX_INMOBI, g_logid_local, private_conf, &run_flag);
	if (errcode != E_SUCCESS)
	{
		FAIL_SHOW;
		run_flag = false;
