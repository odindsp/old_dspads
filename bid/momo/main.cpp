...
static void *doit(void *arg)
{
	...
	int rc;
	FLOW_BID_INFO flow_bid(ADX_MOMO);
	...
	while (run_flag)
	{
		...
		if (strcmp("POST", FCGX_GetParam("REQUEST_METHOD", request.envp)) == 0)
		{
			...
			MY_DEBUG;
			string senddata;
			momo_adapter(ctx, index, recvdata, senddata, flow_bid);
...
int main(int argc, char *argv[])
{
	...
	char *ipbpath = NULL;
	map< uint32_t, vector<string> >::iterator ad_out;         // out
	map<string, vector<uint32_t> >::iterator ad_in;           // in
	...
	err = bid_filter_initialize(g_logid_local, ADX_MOMO, cpu_count, &run_flag, private_conf, thread_id, &ctx);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
	...
	for (ad_out = outputadcat.begin(); ad_out != outputadcat.end(); ++ad_out)
	{
		...
		vector<string> &ad = ad_out->second;
		for (int i = 0; i < ad.size(); ++i)
		{
			printf("%s,", ad[i].c_str());
		}
	...
	err = g_ser_log.init(cpu_count, ADX_MOMO, g_logid_local, private_conf, &run_flag);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
		run_flag = false;
