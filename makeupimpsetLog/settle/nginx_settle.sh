...
tty=$day\/$emouth\/$year
mkdir -p $outpath"/dsp_settle_"$1
echo $tty
while read T1
do
grep $substrde":"$T1 $path$substr$1".log"|awk '{print $1","$4","$7}'|sed "s#\[$substrde:#$1,#g" |sed 's/?/,/g' |awk -F ',' '{print $1" "$2","$3" "$5}'  >$outpath"/dsp_settle_"$1"/dsp_settle_"$T1"0000000"
...
./settle.cgi $outpath"/dsp_settle_"$1"/dsp_impclk_"$T1"0000000"
rm $outpath"/"dsp_settle_$1"/dsp_impclk_"$T1"0000000"
done < tmp_time
