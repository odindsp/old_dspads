...
#define ADVCATTABLE	  "advcat_iqiyi.txt"
#define DEVMAKETABLE  "make_iqiyi.txt"
#define ADVCATPRICETABLE	  "advpricecat_iqiyi.txt"
...
map<uint32_t, vector<uint32_t> > app_cat_table;
map<uint32_t, vector<uint32_t> > adv_cat_table_adxi;
map<uint64_t, vector<uint32_t> > adv_cat_table_price;
...
static void *doit(void *arg)
{
	...
	int errorcode = 0;
	int rc = 0;
	FLOW_BID_INFO flow_bid(ADX_IQIYI);
	...
	while (run_flag)
	{
		...
		if (strcmp("POST", FCGX_GetParam("REQUEST_METHOD", request.envp)) == 0)
		{
			...
			errorcode = iqiyi_adapter(ctx, index, recvdata, senddata, flow_bid);
...
int main(int argc, char *argv[])
{
	...
	map<uint64_t, vector<uint32_t> >::iterator it_price;
	...
	v_cat = adv_cat_table_adxi[200];
	for (uint32_t j = 0; j < v_cat.size(); j++)
		cout << "after mapping is " << v_cat[j] << endl;
#endif

	if (!init_category_table_t((char *)(string(GLOBAL_PATH) + string(ADVCATPRICETABLE)).c_str(), 
		callback_insert_pair_int64, (void *)&adv_cat_table_price, false))
	{
		FAIL_SHOW;
		cflog(g_logid_local, LOGERROR, "init adxcatprice table failed!");
		err = E_PROFILE_INIT_ADVCAT;
		run_flag = false;
		goto exit;
	}
...
#ifdef DEBUG
	for (it_price = adv_cat_table_price.begin(); it_price != adv_cat_table_price.end(); ++it_price)
	{
		string val;
		cout << it_price->first <<" : " ;
		for (int i = 0; i < it_price->second.size(); i++)
		{
			cout << it_price->second[i] << ",";
		}
	...
	err = bid_filter_initialize(g_logid_local, ADX_IQIYI, cpu_count, &run_flag, private_conf, thread_id, &ctx);
	if (err != E_SUCCESS)
	{
	...
	err = g_ser_log.init(cpu_count, ADX_IQIYI, g_logid_local, private_conf, &run_flag);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
		run_flag = false;
