...
static void *doit(void *arg)
{
	...
	int errorcode = 0;
	int rc = 0;
	FLOW_BID_INFO flow_bid(ADX_ADVIEW);
	...
	while (run_flag)
	{
		...
		if (strcmp("POST", FCGX_GetParam("REQUEST_METHOD", request.envp)) == 0)
		{
			...
			char *version = FCGX_GetParam("HTTP_X_ADVIEWRTB_VERSION", request.envp);
			if (version)
			{//version 2.3
				va_cout("find! x-adviewrtb-version: %s", version);
			}
			else
			{
				va_cout("not find! x-adviewrtb-version! ");
				writeLog(g_logid_local, LOGINFO, "not find! x-adviewrtb-version! ");
			...
			if (strcasecmp(contenttype, "application/json") &&
				strcasecmp(contenttype, "application/json; charset=utf-8"))//adview has "charset=utf-8", but firefox has "UTF-8"
			{
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	err = bid_filter_initialize(g_logid_local, ADX_ADVIEW, cpu_count, &run_flag, private_conf, thread_id, &ctx);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
	...
	err = g_ser_log.init(cpu_count, ADX_ADVIEW, g_logid_local, private_conf, &run_flag);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
		run_flag = false;
