...
int main(int argc, char *argv[])
{
	...
	g_datatransfer = new DATATRANSFER[cpu_count];
	...
	*/
	...
	err = bid_filter_initialize(g_logid_local, ADX_AMAX, cpu_count, &run_flag, thread_id, &ctx, private_conf);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW
	...
	{
		if (!connect_log_server(g_logid_local, g_server_ip, g_server_port, &run_flag, &g_datatransfer[i]))
		{
		   FAIL_SHOW
		   run_flag = false;
		   pthread_join(g_datatransfer[i].threadlog, NULL);
		   disconnect_log_server(&g_datatransfer[i]);
		   err = E_LOG_SERVER;
		   goto exit;
		}
		thread_id.push_back(g_datatransfer[i].threadlog);
