...
#define PRIVATE_CONF          "dsp_pxene.conf"
#define DEVMAKETABLE         "make_pxene.txt"
...
static void *doit(void *arg)
{
	...
	int rc;
	FLOW_BID_INFO flow_bid(ADX_PXENE);
...
int main(int argc, char *argv[])
{
	...
	err = bid_filter_initialize(g_logid_local, ADX_PXENE, cpu_count, &run_flag, private_conf, thread_id, &ctx);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
	...
	err = g_ser_log.init(cpu_count, ADX_PXENE, g_logid_local, private_conf, &run_flag);
	if (err != E_SUCCESS)
	{
		FAIL_SHOW;
		run_flag = false;
