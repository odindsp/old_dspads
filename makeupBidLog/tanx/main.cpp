#include <sys/stat.h>
#include <iostream>
#include "../../bid/tanx/tanx_adapter.h"
...
#include "../../bid/tanx/tanx-bidding.pb.h"
#include <signal.h>
#include <syslog.h>
using namespace std;
	...
	if(1)
	{
		...
		while(!out.eof())
		{
			...
			if(bidrequest.ParseFromArray((char *)buf,len))
			{
				...
				parse_common_request(bidrequest,commrequest,bidresponse,nativecreative,viewtype);
				sendLogdirect(datatransfer,commrequest, ADX_TANX, err);
			}
			else
			{
