...
rm  -f $outpath"/record_impclk"$2"_"$1
rm  -f $outpath"/record_settle"$2"_"$1

grep $subkey /data/log/nginx/backup/odin_impclk$2.log >tmp_$1_$2
...
do
echo $T1
grep $substrde":"$T1 "tmp_"$1"_"$2 > "tmp_"$1"_"$2"_"$T1"_impclk"

if [ "$1" == "BAIDU" -o "$1" == "GOYOO" -o "$1" == "ZPLAY" ]
then
grep "mtype=m" "tmp_"$1"_"$2"_"$T1"_impclk" >"tmp_"$1"_"$2"_"$T1"_settle"
fi
while read MAPID
do
countm=`grep  "$MAPID" $outpath"/tmp_"$1"_"$2"_"$T1"_impclk" | grep "mtype=m"|wc -l`
countc=`grep  "$MAPID" $outpath"/tmp_"$1"_"$2"_"$T1"_impclk" | grep "mtype=c"|wc -l`
if [ "$1" == "BAIDU" -o "$1" == "GOYOO" -o "$1" == "ZPLAY" ]
then
spent=`grep "$MAPID" "tmp_"$1"_"$2"_"$T1"_settle"|awk -F '&' '{print $4}' |awk '{print $1}' |awk -F'=' '{sum+=$2}END{print sum}'`
echo $2"_"$T1 $MAPID $spent  >> $outpath"/record_settle"$2"_"$1
fi

echo  $2"_"$T1 $MAPID $countm $countc >>$outpath"/record_impclk"$2"_"$1

done < $mapidFile
rm -f "tmp_"$1"_"$2"_"$T1"_impclk"
